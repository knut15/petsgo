...
import { getCategoryIcon, getCategoryName } from "@/lib/constants";
...
export default function PlaceDetailView({ id }: { id: string }) {
  ...
  if (intro?.barbecue === "가능" || intro?.barbecue === "1")
    facilities.push("바비큐");
  if (intro?.fitness === "있음" || intro?.fitness === "1")
    facilities.push("피트니스");
  if (intro?.sauna === "있음" || intro?.sauna === "1")
    facilities.push("사우나");
  if (intro?.publicbath === "있음" || intro?.publicbath === "1")
    facilities.push("대욕장");
  if (intro?.bicycle === "있음" || intro?.bicycle === "1")
    facilities.push("자전거");
  if (intro?.campfire === "가능" || intro?.campfire === "1")
    facilities.push("캠프파이어");
  if (intro?.seminar === "있음" || intro?.seminar === "1")
    facilities.push("세미나실");
  if (intro?.sports === "있음" || intro?.sports === "1")
    facilities.push("스포츠시설");
  ...
  if (pet?.relaAcdntRiskMtr)
    petRows.push({
      label: "사고 위험",
      value: stripTags(pet.relaAcdntRiskMtr),
      fullWidth: true,
    });
  if (pet?.etcAcmpyInfo)
    petRows.push({
      label: "기타 정보",
      value: stripTags(pet.etcAcmpyInfo),
      fullWidth: true,
    });
  ...
  const headline =
    pet?.acmpyTypeCd ?? (pet ? "반려동물 동반 가능" : "정보 미등록");
                  ...
                  <dl className="rounded-xl border border-stone-200 px-5 py-2 mb-5 divide-y divide-dashed divide-stone-200">
                    {petRows.map((row) =>
                      row.fullWidth ? (
                        ...
                        >
                          <dt className="text-sm text-stone-500 shrink-0">
                            {row.label}
                          </dt>
